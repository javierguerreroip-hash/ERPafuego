...
export async function createHandler(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    res.status(201).json(await taxRateService.createTaxRate(input));
  } catch (error) {
    next(error);
  }
}
...
export async function updateHandler(req: Request, res: Response, next: NextFunction) {
  try {
    const input = taxRateSchema.parse(req.body);
    res.json(await taxRateService.updateTaxRate(req.params.id, input));
  } catch (error) {
    next(error);
  }
}
...
export async function setActiveHandler(req: Request, res: Response, next: NextFunction) {
  try {
    res.json(await taxRateService.setTaxRateActive(req.params.id, Boolean(req.body.active)));
  } catch (error) {
    next(error);
  }
}
