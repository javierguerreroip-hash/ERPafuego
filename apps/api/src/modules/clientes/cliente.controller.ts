...
export async function createHandler(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    res.status(201).json(await clienteService.createCliente(input));
  } catch (error) {
    next(error);
  }
}
...
export async function updateHandler(req: Request, res: Response, next: NextFunction) {
  try {
    const input = clienteSchema.parse(req.body);
    res.json(await clienteService.updateCliente(req.params.id, input));
  } catch (error) {
    next(error);
  }
}
...
export async function setActiveHandler(req: Request, res: Response, next: NextFunction) {
  try {
    res.json(await clienteService.setClienteActive(req.params.id, Boolean(req.body.active)));
  } catch (error) {
    next(error);
  }
}
